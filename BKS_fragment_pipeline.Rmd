...
T004_fits <- list("model_tmas" = bind_rows(T004_batch_1_fits$model_tmas %>% left_join(make_layout(layout_path_1), by = c("well", "condition")), 
                                           T004_batch_2_fits$model_tmas %>% left_join(make_layout(layout_path_2), by = c("well", "condition"))),
                  "dRFU_tmas" = bind_rows(T004_batch_1_fits$dRFU_tmas %>% left_join(make_layout(layout_path_1), by = c("well", "condition")), 
                                          T004_batch_2_fits$dRFU_tmas %>% left_join(make_layout(layout_path_2), by = c("well", "condition"))),
                  "df_BIC" = bind_rows(T004_batch_1_fits$df_BIC %>% left_join(make_layout(layout_path_1), by = c("well", "condition")), 
                                       T004_batch_2_fits$df_BIC %>% left_join(make_layout(layout_path_2), by = c("well", "condition"))))

SYPRO_fits <- list("model_tmas" = bind_rows(SYPRO_batch_1_fits$model_tmas %>% left_join(make_layout(layout_path_1), by = c("well", "condition")), 
                                            SYPRO_batch_2_fits$model_tmas %>% left_join(make_layout(layout_path_2), by = c("well", "condition"))),
                  "dRFU_tmas" = bind_rows(SYPRO_batch_1_fits$dRFU_tmas %>% left_join(make_layout(layout_path_1), by = c("well", "condition")), 
                                          SYPRO_batch_2_fits$dRFU_tmas %>% left_join(make_layout(layout_path_2), by = c("well", "condition"))),
                  "df_BIC" = bind_rows(SYPRO_batch_1_fits$df_BIC %>% left_join(make_layout(layout_path_1), by = c("well", "condition")), 
                                       SYPRO_batch_2_fits$df_BIC %>% left_join(make_layout(layout_path_2), by = c("well", "condition"))))
...
all_fits$model_tmas %>%
  filter(compound_group != "Empty",
         which_model == "s1_d_pred",
         dye == "T004") %>%
  ggplot(aes(x = condition, y = mean_tma1, color = compound_group, shape = concentration %>% as.character(), size = concentration %>% as.character(), group = compound)) + # 
  #scale_shape_manual(values = c("DMSO" = 1, "ADP_ribose" = 1, "test" = 16)) +
