...
```{r}
MAIN_PATH <- "data/Exp0858--20200729_Nsp3_fragment_test_batch_2_with_SYPRO_and_T004/"
LAYOUT_PATH <- MAIN_PATH %>% paste0("Exp0858--20200729_Nsp3_fragment_test_batch_2_with_SYPRO_and_T004_layout.csv" )
DATA_PATH <- MAIN_PATH %>% paste0("Exp0858--20200729_Nsp3_fragments_batch2_screen_SYPRO_and_T004.csv" )
SAVE_PATH <- MAIN_PATH %>% paste0("out/" )


layout <- make_layout(LAYOUT_PATH)

df_raw <- read_qTower(DATA_PATH) %>%
                                    mutate(Temperature = .$Temperature + (25-1))%>%
                                      full_join(layout)   %>%
                                    unite(variable, c(well, channel), remove = FALSE) %>%
                  group_by(variable)  %>%
                ...
                mutate(value_norm_by_comp = BBmisc::normalize(value, method = "range", range = c(0,1))) %>%
                ungroup()
