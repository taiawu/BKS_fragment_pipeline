...
T004_batch_1$df_raw <- swap_layout(T004_batch_1$df_raw, LAYOUT_PATH %>% paste0("Exp0865.1--20200811_Nsp3_mac1_fragments_batch1_layout.csv"))
SYPRO_batch_1$df_raw <- swap_layout(SYPRO_batch_1$df_raw, LAYOUT_PATH %>% paste0("Exp0865.1--20200811_Nsp3_mac1_fragments_batch1_layout.csv"))

T004_batch_2$df_raw <- swap_layout(T004_batch_2$df_raw, LAYOUT_PATH %>% paste0("Exp0865.2--20200811_Nsp3_mac1_fragments_batch2_layout.csv"))
SYPRO_batch_2$df_raw <- swap_layout(SYPRO_batch_2$df_raw, LAYOUT_PATH %>% paste0("Exp0865.2--20200811_Nsp3_mac1_fragments_batch2_layout.csv"))
```
...
Checking up on each of the datasets
```{r}
...
T004_batch_1$df_raw %>% head()

T004_batch_1$by_variable %>% 
  group_by(well) %>%
  select(well, channel_f) %>%
  tally() # 5 channels for everything


T004_batch_1$by_variable %>%
  group_by(compound_group) %>%
  tally() %>%
  mutate(per_channel = n/5) %>%
  mutate(with_protein = per_channel/2) # # looks right: 4 of each control + 120 test compounds

T004_batch_2$by_variable %>%
  group_by(compound_group) %>%
  tally() %>%
  mutate(per_channel = n/5) %>%
  mutate(with_protein = per_channel/2) # looks right: 6 of ecah control + 150 test compounds 

SYPRO_batch_1$by_variable %>%
  group_by(compound_group) %>%
  tally() %>%
  mutate(per_channel = n/5) %>%
  mutate(with_protein = per_channel/2) # looks right: 4 of each control + 120 test compounds 

SYPRO_batch_2$by_variable %>%
  group_by(compound_group) %>%
  tally() %>%
  mutate(per_channel = n/5) %>%
  mutate(with_protein = per_channel/2) # looks right: 6 of ecah control + 150 test compounds 
...
T004_batch_1$fits$s1_list$tm_models_all %>%
  left_join(make_layout(layout_path_1), by = c("well", "condition")) %>%
  group_by(compound_group) %>%
  tally() # 4, 4, 120
...
T004_batch_1$fits$s1_list$tm_models_all %>%
  left_join(make_layout(layout_path_1), by = c("well", "condition")) %>%
  distinct() %>%
  nrow() # 136: with 8 empties, this checks out

T004_batch_1$fits$s1_list$tm_models_all %>%
  left_join(make_layout(layout_path_1), by = c("well", "condition")) %>%
  group_by(compound_group) %>%
  tally() # 4, 4, 120, 8 empties


T004_batch_1$fits$s1_d_list$tm_models_all %>%
  left_join(make_layout(layout_path_1), by = c("well", "condition")) %>%
  distinct() %>%
  nrow() # 136: with 8 empties, this checks out
...
T004_batch_2$fits$s1_d_list$tm_models_all %>%
  left_join(make_layout(layout_path_2), by = c("well", "condition")) %>%
  distinct() %>%
  nrow() # 162: with 8 empties, this checks out

T004_batch_2$fits$s1_d_list$tm_models_all %>%
  left_join(make_layout(layout_path_2), by = c("well", "condition")) %>%
  group_by(compound_group) %>%
  tally() # 6, 6, 150, 0 empties
```
...
T004_batch_2_fits$dRFU_tmas
T004_batch_2_fits$model_tmas
...
This should be better functionalized, but is code worth continuing to work on!! don't just rewrite this bit!
```{r}
#### add this to the final processing script, ultimately! This step should not be generated---it is clearning up a data org. mess made by poor planning in an earlier function
merge_elements <- function(in_list, element) {
  
   lapply(c(4:7), function(x) in_list$fits[[x]][[element]])    %>%
     bind_rows()
 
}

merge_and_add_layout <- function(batch_list, element_number, layout_file, join_layout_by) {
  out_df <- merge_elements(batch_list, element_number) %>%
              left_join(make_layout(layout_file), by = join_layout_by) #make_layout(LAYOUT_PATH %>% paste0("Exp0865.1--20200811_Nsp3_mac1_fragments_batch1_layout.csv")), by = "condition")
}



t4_tms <- bind_rows( merge_and_add_layout(batch_list = T004_batch_1, 
                                 element_number = 4, 
                                 layout_file = layout_path_1, 
                                 join_layout_by = "condition"),
                     
                     merge_and_add_layout(batch_list = T004_batch_2, 
                                 element_number = 4, 
                                 layout_file = layout_path_2, 
                                 join_layout_by = "condition")) %>%
                      filter(compound_group != "Empty")%>%
  distinct(condition, well, which_model, .keep_all = TRUE) 

t4_BICs <- bind_rows( merge_and_add_layout(batch_list = T004_batch_1, 
                                 element_number = 2, 
                                 layout_file = layout_path_1, 
                                 join_layout_by = c("well","condition")),
...
SYPRO_tms <- bind_rows( merge_and_add_layout(batch_list = SYPRO_batch_1, 
                                 element_number = 4, 
                                 layout_file = layout_path_1, 
                                 join_layout_by = "condition"),
                     
                     merge_and_add_layout(batch_list = SYPRO_batch_2, 
                                 element_number = 4, 
                                 layout_file = layout_path_2, 
                                 join_layout_by = "condition"))%>%
                      filter(compound_group != "Empty")%>%
...
SYPRO_BICs <- bind_rows( merge_and_add_layout(batch_list = SYPRO_batch_1, 
                                 element_number = 2, 
                                 layout_file = layout_path_1, 
                                 join_layout_by = c("well","condition")),
...
SYPRO_fits <- left_join(SYPRO_tms, SYPRO_BICs %>% select(condition, which_model, BIC, well), by = c("well","condition", "which_model")) %>% ungroup()
T004_fits <- left_join(t4_tms, t4_BICs %>% select(condition, which_model, BIC, well), by = c("well","condition", "which_model")) %>% ungroup()

# make sure everything has the same number of rows
t4_BICs %>% nrow() == t4_tms %>% nrow() 
t4_tms %>% nrow() == T004_fits %>% nrow() 
SYPRO_tms %>% nrow() == SYPRO_BICs %>% nrow() 
SYPRO_BICs %>% nrow() == SYPRO_fits %>% nrow()
```
